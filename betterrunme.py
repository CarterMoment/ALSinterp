...
import joblib  # To load the saved LabelEncoder

# Load the trained gesture recognition model
model = tf.keras.models.load_model('gesture_recognition_model.keras')

# Load the saved LabelEncoder
label_encoder = joblib.load('gesture_label_encoder.pk1')
...
while True:
    ...
    if frame_count % 2 == 0:  # Skip every 2nd frame for performance optimization
        continue
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            # Convert the predicted index back to the gesture label using the saved LabelEncoder
            gesture_label = label_encoder.inverse_transform([gesture_index])[0]
